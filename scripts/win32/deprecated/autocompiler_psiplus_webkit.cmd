...
ECHO D | xcopy psi\themes themes /E
ECHO MOVE /Y psi-plus.exe psi-plus-portable.exe ^&^& DEL make-psi-plus-portable.bat>make-psi-plus-portable.bat
ECHO Archiving build
CALL 7z a -tzip -scsDOS -mx9 "psi-plus-0.15.%revision%-webkit-win32.zip" "themes" "make-psi-plus-portable.bat" "psi-plus.exe" "readme.txt"
...
CALL googlecode_upload.py --user your_google_login --password your_googlecode_password --project psi-dev --summary "Psi+ WebKit Nightly Build (Beta) || psi-git %date% %currentTime% MSD || Qt 4.7.1 || Win32 OpenSSL Libs v0.9.8q" --labels "WebKit,Windows,Archive" "psi-plus-0.15.%revision%-webkit-win32.zip"
ECHO Completed
GOTO :cleaning
